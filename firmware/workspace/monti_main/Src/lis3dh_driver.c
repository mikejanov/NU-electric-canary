...
u8_t LIS3DH_ReadReg(u8_t Reg, u8_t *Data) {
  ...
  rslt = HAL_I2C_Master_Transmit_IT(&hi2c1, LIS3DH_MEMS_I2C_ADDRESS<<1, &Reg, sizeof(Reg));

  // Begin to read from I2C for length of Data
  rslt = HAL_I2C_Master_Receive_IT(&hi2c1, LIS3DH_MEMS_I2C_ADDRESS<<1, Data, sizeof(Data));
...
u8_t LIS3DH_WriteReg(u8_t WriteAddr, u8_t Data) {
  ...
  rslt = HAL_I2C_Master_Transmit_IT(&hi2c1, LIS3DH_MEMS_I2C_ADDRESS<<1, &WriteAddr, sizeof(Data));
...
/*******************************************************************************
* Function Name		: LIS3DH_Monti_Init
* Description		: Generic init of LIS3DH for Monti
* Input			: None
* Output		: None
* Return		: None
*******************************************************************************/
u8_t LIS3DH_Monti_Init() {
	uint8_t check;
...
/*******************************************************************************
* Function Name		: LIS3DH_Monti_Get_Data
* Description		: Get raw axis information and orienaton ints?
* Input			: AxesRaw_t *data
* Output		: None
* Return		: None
*******************************************************************************/
u8_t LIS3DH_Monti_Get_Raw_Data(AxesRaw_t *data) {

	/*
	// todo: interupts specific to orientations?
	// 6D array = UP_SX, UP_DX, DW_SX, DW_DX, TOP, BOTTOM
	u8_t positions[6] = {0, 0, 0, 0, 0, 0};
	// Go through each axis and get data
